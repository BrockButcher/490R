...
For a nomianl dependent variable with k categories, the multinomial regression model estimates k-1 logit equations. Logit is commonly used as the link function in ordinal regression. This predicts probabilities of outcome <=i, $p_i$. $y_i=p_i+Error$ 

$Log[p_i/(1-p_i)]=a_i_b_1x_1+b_2x_2+...$
Where $p_i=$ probability of an outcome <=i
$a_i=$ intercept for outcome <=i
...
```{r}
feetreg <- read.csv('feetreg')
library(MASS)
sirefact <- as.factor(SIRE)
asfeet <- as.factor(Hfeet)

cbind(sirefact,asfeet,feetreg)

model <- polr(asfeet~Dfeet+MGDfeet+Dfeet*MGDfeet, Hess = TRUE, data = feetreg)
summary(model)
```
